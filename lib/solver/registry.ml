...
let validate_input year day part =
  let min_year = 2015
  and max_year = 2024 in
  let min_day = 1
  and max_day = 25 in
  let min_part = 1
  and max_part = 2 in
  if year < min_year || year > max_year
  then raise (Year_out_of_range year)
  else if day < min_day || day > max_day
  then raise (Day_out_of_range year)
  else if part < min_part || part > max_part
  then raise (Part_out_of_range part)
  else year, day, part
;;
