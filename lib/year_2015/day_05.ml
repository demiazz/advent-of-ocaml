...
module Part_one = struct
  ...
  type state = { vowels_count : int; has_twice : bool }
  ...
  let detect_vowel pair state =
    if is_vowel (snd pair) then
      { state with vowels_count = state.vowels_count + 1 }
    else state

  let is_twice = function Some x, y when x = y -> true | _ -> false
  ...
  let is_nice string =
    ...
    let rec validate state =
      match next () with
      | Some pair ->
          if is_bad_pair pair then false
...
module Part_two = struct
  ...
  let detect_char_repeat triple state =
    if state.has_char_repeat then state
    else
      match triple with
  ...
  let detect_pair_repeat triple state =
    if state.has_pair_repeat then state
    else
      match triple with
      | _, Some x, y ->
          let is_repeat =
            if x = y && state.protect_from_overlap then false
            else PairSet.mem (x, y) state.seen
          in
          if is_repeat then { state with has_pair_repeat = true }
          else
            {
              state with
      ...
      | _ ->
          if state.protect_from_overlap then
            { state with protect_from_overlap = false }
          else state
  ...
  let is_nice string =
    ...
    let rec validate state =
      ...
      | Some triple ->
          ...
          in
          if next_state.has_char_repeat && next_state.has_pair_repeat then true
