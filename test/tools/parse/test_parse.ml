...
module Lines_of = struct
  let process_line line =
    match line with "true" -> Some true | "false" -> Some false | _ -> None
  ...
  let test_correct_input name input expected =
    let run () =
      Alcotest.(check (list bool)) "are equal" expected (handle input)
    in
    Alcotest.test_case name `Quick run

  let test_incorrect_input name input =
    let run () =
      Alcotest.check_raises "unknown lines raises exception" Parse.Invalid_input
