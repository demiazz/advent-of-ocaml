...
let zero_test_cases =
  ...
  let make name =
    let run () = Alcotest.(check pair_t) "are equal" (Point.create 0 0) Point.zero in
...
let compare_test_cases =
  let samples =
    ...
    ; "returns 1 when coordinates when x of first point is greater", (5, 2), (4, 2), 1
    ; "returns 1 when coordinates when x of first point is greater", (5, 2), (4, 2), 1
    ; "returns 1 when coordinates when x of first point is greater", (5, 3), (4, 2), 1
    ; "returns -1 when coordinates when x of first point is lesser", (3, 2), (4, 2), -1
    ; "returns -1 when coordinates when x of first point is lesser", (3, 2), (4, 2), -1
    ; "returns -1 when coordinates when x of first point is lesser", (3, 3), (4, 2), -1
    ; "returns 1 when coordinates when y of first point is greater", (4, 3), (4, 2), 1
    ; "returns -1 when coordinates when y of first point is lesser", (3, 2), (4, 2), -1
    ]
  in
...
let equal_test_cases =
  let samples =
    [ "returns true when coordinates of both points are equal", (4, 2), (4, 2), true
...
let sub_value_test_cases =
  let samples = [ "substracts a value to both coordinates", (4, 2), 2, (2, 0) ] in
  point_value_test_cases Point.( - ) samples
;;

let mul_value_test_cases =
  let samples = [ "multiplies both coordinates on a value", (4, 2), 2, (8, 4) ] in
  point_value_test_cases Point.( * ) samples
;;

let div_value_test_cases =
  let samples = [ "divides both coordinates on a value", (4, 2), 2, (2, 1) ] in
  point_value_test_cases Point.( / ) samples
;;
...
let add_points_test_cases =
  let samples = [ "adds a value to both coordinates", (2, 0), (-4, 2), (-2, 2) ] in
  points_test_cases Point.( ++ ) samples
;;

let sub_points_test_cases =
  let samples = [ "substracts a value to both coordinates", (2, 0), (-2, -2), (4, 2) ] in
  points_test_cases Point.( -- ) samples
;;

let mul_points_test_cases =
  let samples = [ "multiplies a value to both coordinates", (2, 4), (-4, 2), (-8, 8) ] in
  points_test_cases Point.( ** ) samples
;;

let div_points_test_cases =
  let samples = [ "divides a value to both coordinates", (8, 4), (-4, 2), (-2, 2) ] in
  points_test_cases Point.( // ) samples
;;
