...
let zero_test_cases =
  ...
  let make name =
    let run () = Alcotest.(check pair_t) "are equal" (Point.create 0 0 0) Point.zero in
...
let compare_test_cases =
  let samples =
    [ "returns 0 when coordinates of both points are equal", (4, 2, 5), (4, 2, 5), 0
  ...
  let make (name, left, right, expected) =
    let run () =
      let actual = Point.compare (Point.of_triple left) (Point.of_triple right) in
...
let equal_test_cases =
  let samples =
    [ "returns true when coordinates of both points are equal", (4, 2, 6), (4, 2, 6), true
...
let min_x_test_cases =
  let samples =
    ...
    ; "returns right x coordinate when right is lesser", (4, 0, 0), (3, 0, 0), 3
    ]
  in
...
let min_y_test_cases =
  let samples =
    ...
    ; "returns right y coordinate when right is lesser", (0, 4, 0), (0, 3, 0), 3
    ]
  in
...
let min_z_test_cases =
  let samples =
    ...
    ; "returns right z coordinate when right is lesser", (0, 0, 4), (0, 0, 3), 3
    ]
  in
...
let add_value_test_cases =
  let samples = [ "adds a value to both coordinates", (2, 0, -4), 2, (4, 2, -2) ] in
  point_value_test_cases Point.( + ) samples
;;

let sub_value_test_cases =
  let samples = [ "substracts a value to both coordinates", (4, 2, -4), 2, (2, 0, -6) ] in
  point_value_test_cases Point.( - ) samples
;;

let mul_value_test_cases =
  let samples = [ "multiplies both coordinates on a value", (4, 2, -4), 2, (8, 4, -8) ] in
  point_value_test_cases Point.( * ) samples
;;

let div_value_test_cases =
  let samples = [ "divides both coordinates on a value", (4, 2, -4), 2, (2, 1, -2) ] in
  point_value_test_cases Point.( / ) samples
;;
...
let sub_points_test_cases =
  let samples =
    [ "substracts a value to both coordinates", (2, 0, -4), (-2, -2, 8), (4, 2, -12) ]
  in
  points_test_cases Point.( -- ) samples
;;
...
let mul_points_test_cases =
  let samples =
    [ "multiplies a value to both coordinates", (2, 4, -4), (-4, 2, 8), (-8, 8, -32) ]
  in
  points_test_cases Point.( ** ) samples
;;
...
let div_points_test_cases =
  let samples =
    [ "divides a value to both coordinates", (8, 4, -4), (-4, 2, 2), (-2, 2, -2) ]
  in
  points_test_cases Point.( // ) samples
;;
...
let to_string_test_cases =
  let samples =
    [ "creates string representation of point", (8, 5, -4), "{ x = 8; y = 5; z = -4 }" ]
  in
  let make (name, initial, expected) =
    let run () =
