...
module File_input = struct
  let kind = "file"
  let remove file_name = try Sys.remove file_name with _ -> ()
...
module Suite (Input : Input) = struct
  let test_chars_of name (input, expected) =
    ...
    let run () =
      input |> actual |> Alcotest.(check string) "are equal" expected
    in
  ...
  let test_bytes_of name (input, expected) =
    ...
    let actual input = Input.from_input input handle in
    let run () =
      input |> actual |> Alcotest.(check (list int)) "are equal" expected
    in
    Alcotest.test_case name `Quick run
  ...
  let test_lines_of name (input, expected) =
    ...
    let actual input = Input.from_input input handle in
    let run () =
      input |> actual |> Alcotest.(check (list string)) "are equal" expected
    in
    Alcotest.test_case name `Quick run
